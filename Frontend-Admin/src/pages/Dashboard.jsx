 ...
 *
 * Purpose:
 * - Visualize police offices and active checkpoints on a map
 * - Provide quick filters for "all" vs "our" data (based on the logged-in admin)
 *
 * Backend endpoints used:
 * - GET /admin/map/data/
 * - GET /geocode/reverse/?lat=...&lng=... (on right-click)
 *
 * Notes:
 * - Map data is refreshed on a fixed interval (30s) to keep the UI updated without spamming.
 */
export default function Dashboard() {
  ...
  useEffect(() => {
    // Step 1: Fetch map data (offices + active checkpoints) from the backend.
    // This is intentionally isolated into a function so we can also reuse it for polling.
    const loadMapData = () => {
      setLoadError('')
      api.getAdminMapData()
        .then((data) => {
          // Backend shape: { police_offices: [...], active_checkpoints: [...] }
        ...
        .catch((err) => {
          // If auth is invalid, the interceptor will redirect; this message is for other failures.
    ...
    // Step 3: Polling refresh (keeps dashboard “live” without spamming the backend)
  ...
  const ourOfficeIds = useMemo(() => {
    // "Our" data is based on which offices were created by the current admin.
  ...
  const visibleOffices = useMemo(() => {
    // Office filter: show all offices or only those created by the current admin.
  ...
  const visibleCheckpoints = useMemo(() => {
    // Checkpoint filter: "our" means checkpoints belonging to offices created by this admin.
  ...
  const markers = useMemo(() => {
    // Normalize to a single marker list so the map component only needs one prop.
    // We add a `type` field so `GoogleMap` can choose the correct icon.
    const out = []
    if (showOffices) {
  ...
  async function handleMapRightClick(lat, lng) {
    // Admin dashboard helper: right-click any point and show the reverse-geocoded address.
  ...
  async function copy(text) {
    // Copy helper: used by right-click info panel.
    try {

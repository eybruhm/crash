 ...
 *
 * Purpose:
 * - Authenticate an admin user and persist auth state in localStorage.
 *
 * Backend endpoint used:
 * - POST /auth/login/
 *   Returns: { role, user, access, refresh, message }
 *
 * Notes:
 * - `role` is stored along with the user so `RequireAuth` can gate access.
 */
export default function Login() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // Store tokens + user (include role for RequireAuth gating)
      if (data?.access) {
      ...
      // Redirect back to the page the user originally attempted to open.

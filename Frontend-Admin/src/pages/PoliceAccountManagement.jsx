 ...
 *
 * Purpose:
 * - Admin-only CRUD UI for police offices/accounts (tbl_police_offices)
 *
 ...
 *
 * Notes:
 * - Password is provided in plaintext only when creating/changing credentials; backend hashes it.
 * - Some optional fields must be omitted or set to null; sending empty strings can trigger DRF errors.
 */
export default function PoliceAccountManagement() {
  ...
  function norm7(value) {
    // Coordinates are stored with limited precision; keeping this consistent prevents "jitter".
  ...
  useEffect(() => {
    // Step 1: Refresh table when scope changes.
    load()
  }, [scope])

  function load() {
    // scope=all: all offices in the system; scope=our: offices created by this admin
    setLoading(true)
  ...
  function openAddModal() {
    // Reset the form every time the modal opens so the user starts clean.
  ...
  function openEditModal(p) {
    ...
    // Password is intentionally blank (only set if the admin wants to change it).
  ...
  async function handleAdd(e) {
    ...
    // Step 2: Client-side validation (fast feedback). Backend still enforces the real rules.
    ...
    setSaving(true)
    try {
      // Step 3: Ensure we have the admin_id; backend uses it for created_by.
      ...
      // Build payload:
      // - Avoid empty strings for optional CharFields (backend may reject them).
      // - Normalize coordinates to 7 decimals for consistent storage.
      const payload = {
        email: addForm.email,
      ...
      // Step 4: Refresh list and close modal after a short success delay.
  ...
  async function handleEdit(e) {
    ...
    // Step 2: Validate only the fields that are changing.
    ...
    setSaving(true)
    try {
      // Step 3: Confirm admin_id still exists (helps when storage was cleared/corrupted).
      const admin = getStoredUser()
      if (!admin?.admin_id) {
        throw new Error('Missing admin_id. Please log out and log in again.')
      }
      ...
      // Use nulls for optional fields so the backend clears them cleanly.
      const updates = {
  ...
  async function handleDelete(id) {
    ...
    try {
      ...
      // Error handled silently (table remains unchanged).
    } finally {
      setDeletingId(null)
    }
  }

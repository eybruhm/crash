 ...
 *
 * Purpose:
 * - Utility for IT/admin workflows where a Django-compatible password hash is needed for
 *   direct DB insertion (e.g., migrating/repairing records in Supabase).
 *
 * Backend endpoint used:
 * - POST /admin/password-hash/
 *   Backend uses Django's `make_password()` to produce the PBKDF2 hash.
 *
 * Security notes:
 * - The plaintext password is sent to the backend over HTTP(S) for hashing.
 * - Use only in trusted environments (local dev / internal tooling), and protect logs.
 */
export default function PasswordHashConverter() {
  ...
  /**
   * Convert plaintext password to a Django-compatible hash.
   * Uses the backend endpoint that runs Django's `make_password()`.
   */
  async function handleConvert() {
    ...
    // Reset UI state for a fresh conversion attempt.
    setLoading(true)
    setError('')
    setHashedPassword('')
    setCopied(false)
    ...
    try {
      // Step 1: Call backend endpoint to hash password.
      // (This is not a cryptographic operation done in the browser.)
      const response = await apiClient.post('/admin/password-hash/', {
        password: password,
      })

      // Step 2: Store result in UI.

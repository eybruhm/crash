 ...
 * - Allow profile edits and password changes
 *
 ...
 *
 * Notes:
 * - Some backend profile responses do not include `role`, but the frontend guard needs it,
 *   so we preserve role when updating localStorage.
 */
export default function Profile() {
  const navigate = useNavigate()

  // Stored user is the fastest initial render source; we refresh from backend on mount.
  ...
  // Keep storage in sync so other pages/components read the latest username/email.
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        // Step 1: Get authoritative profile from backend.
        ...
        // Step 2: Seed the edit form so the UI can toggle into edit mode immediately.
        setEditForm({
  ...
  useEffect(() => {
    // Keep edit form synced if profile changes (e.g., after saving).
    if (profile) {
      setEditForm({
  ...
  async function handleSaveDetails() {
    ...
    setSaving(true)
    try {
      // Step 1: Build backend payload. Use null for optional contact to clear the value.
      const payload = {
      ...
      // Step 2: Persist and update local UI state.
  ...
  async function handleChangePassword() {
    ...
    // Step 1: Client-side validation (backend still enforces policy).
    ...
    setSaving(true)
    try {
      // Step 2: Call backend; interceptor handles auth refresh/redirect if needed.

...
/**
 * Manual Report Insertion (Admin)
 *
 * Used when:
 * - Admin needs to manually encode a 911/offline report into the system.
 *
 * Backend endpoints used:
 * - GET /admin/police-offices/?scope=all (for selecting assigned office)
 * - GET /admin/users/search/?q=... (optional: link report to a user)
 * - POST /admin/reports/manual/ (creates the report)
 *
 * Notes:
 * - `assigned_office` is required because dispatch must decide ownership of the incident.
 * - datetime-local inputs have no timezone; we encode explicit +08:00 (Asia/Manila).
 */
...
export default function ManualReport() {
  ...
  const sortedOffices = useMemo(() => {
    // Keep office list stable and human-friendly (alphabetical) for faster selection.
  ...
  useEffect(() => {
    // Step 1: Load all police offices so the admin can choose `assigned_office`.
  ...
  // Prefill timestamps (editable): speeds up manual encoding work.
  useEffect(() => {
    const now = new Date()
  ...
  function update(k, v) {
    // Single update helper so input handlers stay small and consistent.
    setForm((s) => ({ ...s, [k]: v }))
  }

  function norm7(value) {
    // Normalize coordinates to 7 decimal places to match backend precision.
  ...
  async function handleSearchUsers() {
    // Calls the backend search helper and displays results in the User Search tab.
  ...
  // Dynamic search: as-you-type (keeps UX consistent with other search tabs)
  useEffect(() => {
  ...
  function selectUser(u) {
    // Selecting a user is optional; we store the UUID in `reporter`.
    setSelectedUser(u)
  ...
  function clearUser() {
    // Remove the linked reporter.
    setSelectedUser(null)
    update('reporter', '')
  }

  const filteredOffices = useMemo(() => {
    // Police search tab: filter by office name or location substring.
  ...
  function selectOffice(o) {
    // Assigned office is REQUIRED for 911/offline insertion.
    setSelectedOffice(o)
  ...
  function clearOffice() {
    // Clears the required assigned office field.
  ...
  async function handleSubmit(e) {
    ...
    // Step 2: Validate required fields before calling the backend.
    if (!form.category) {
    ...
    setSaving(true)
    try {
      // Build payload with the same shape the backend expects.
      const payload = {
      ...
      // created_at / updated_at are optional overrides for offline encoding.
      ...
      // Step 3: Reset form state after a successful insert.
      setForm((s) => ({
        ...s,
        category: '',
        status: 'Pending',

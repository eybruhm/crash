 ...
 *
 * Used by:
 * - Map components that need `window.google.maps` but must not inject the script multiple times.
 *
 * Behavior:
 * - If already loaded, resolves immediately with `window.google.maps`.
 * - If a script tag already exists (maybe inserted by index.html or another component), it polls
 *   until `window.google.maps` becomes available.
 * - Otherwise, injects the script and uses a `callback=` param.
 * - Includes timeouts + fallbacks so UI can show a meaningful error instead of hanging forever.
 */
export function loadGoogleMapsApi(apiKey, options = {}) {
  ...
  googleMapsPromise = new Promise((resolve, reject) => {
    // Hard timeout: avoid the UI waiting forever if the network/key is misconfigured.
    ...
    // If the script was injected elsewhere, don't inject again; just wait for it.
    ...
    // Create a unique global callback name for this load attempt.
    const callbackName = `initGoogleMaps_${Date.now()}`
    let callbackFired = false
    
    // Set up the global callback referenced by `callback=` in the URL.
    window[callbackName] = () => {
      callbackFired = true
      if (window.google?.maps) {
    ...
    // Build URL with callback + optional libraries.
    ...
    // Fallback: poll for `window.google.maps` if callback doesn't fire (some blockers prevent callbacks).
    let pollAttempts = 0
    ...
    script.onload = () => {
      // Give callback a moment to fire before declaring success via polling.
      setTimeout(() => {

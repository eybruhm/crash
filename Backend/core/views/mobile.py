...
class MobileCreateReportWithMediaAPIView(APIView):
    ...
    def post(self, request):
        # Parse primary report fields from multipart/form-data
        ...
        # Validate required fields early (fail fast before doing any DB/storage work)
        ...
        # Validate reporter UUID format (tbl_users.user_id)
        try:
        ...
        # Collect uploaded files (supports either multiple files or a single file field)
        uploaded = []
        files = []
        ...
        # Storage configuration + server-side upload limits
        # - Bucket is configurable via env
        # - Limits are hardcoded in services.py so the backend enforces rules even if env is missing
        ...
        # Validate ALL files first so we don't create a report and then fail mid-upload.
        ...
        # Classify each file as image/video based on content-type, and enforce max size.
        for f in files:
            # Size limit
        ...
        # Enforce per-report media limits (prevents spam and keeps storage costs predictable)
        ...
        # Derive human-readable location and select the nearest police office
        # (same concept as ReportViewSet.perform_create)
        ...
        # Create the report row after all validation passes
        ...
        for item in normalized:
            ...
            # Storage path scheme (keeps evidence grouped per report):
            # reports/<report_id>/<image|video>/<random_filename>
            ...
            # If upload fails, raise a ValidationError so the client sees a clear reason.
            try:
            ...
            # Get a URL that the web apps can use to display/download the evidence
            supabase = get_supabase_client()
            public_url = supabase.storage.from_(bucket).get_public_url(storage_path)
            if isinstance(public_url, dict):
                public_url = public_url.get('publicUrl') or public_url.get('public_url')

            # Create tbl_media row (metadata) for retrieval by report_id
        ...
        # Response shape is intentionally minimal: identifiers + geocoded location + uploaded media list
        return Response(
            {

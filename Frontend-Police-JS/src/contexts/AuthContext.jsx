...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    const initAuth = () => {
      // Validate tokens in local storage to determine auth state
      if (checkAuth()) {
        // Hydrate user state from stored profile
  ...
  const login = async (email, password, role = 'police') => {
    ...
    try {
      // Execute login against backend
      const result = await apiLogin(email, password, role)
      
      // Update global context state on success
      if (result.success) {
        setUser(result.user)
        return true
      }
      ...
      // Capture and expose error to UI components

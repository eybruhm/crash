...
const buildQueryString = (filters = {}) => {
  ...
  // Conditionally append filters if they are active (not null/'all')
  // This prevents sending empty parameters to the backend
  ...
  // Transform category format to match backend expectations (capitalized)
  // e.g. "violence" -> "Violence"
...
export const exportAnalyticsPDF = async (filters = {}) => {
  try {
    ...
    // Request blob response type for PDF file download
    // Using apiClient ensures proper Authorization headers are included
    ...
    // Intent: Open PDF in new tab
    const win = window.open(blobUrl, '_blank');
    
    // Handle popup blockers: if window.open fails, fallback to direct link click
    if (!win) {
    ...
    // Clean up memory: blob URL can be revoked after a short delay
    // This releases the reference to the file in the browser's memory

...
const normalizeSenderTypeForBackend = (senderType) => {
  ...
  // Map various inputs to allowed backend values
...
export const sendMessage = async (reportId, content, senderId, senderType, receiverId) => {
  try {
    // Ensure sender type matches allowed backend values ('police' or 'user')
    ...
    // Log detailed diagnostics to facilitate debugging message failures
    ...
    // Propagate backend error details if available for UI display

 ...
 * LIVE MAP PAGE
 * ============================================================================
 * Purpose: Tactical view for real-time situational awareness.
 * Features:
 * - Google Maps integration showing active reports & checkpoints
 * - Filter toggle for layers (Reports/Checkpoints)
 * - Map marker management (Checkpoints CRUD)
 * - Reverse geocoding tool (Right-click map)
...
const MapPage = () => {
  ...
  const [selectedPin, setSelectedPin] = useState(null) // Generic pin selection
  ...
  // Triggers visual marker + reverse geocoding API lookup
  const handleMapRightClick = async (e) => {
    const lat = e.latLng.lat()
    const lng = e.latLng.lng()
    
    // Set context menu with loading state immediately
    setContextMenu({
      lat,
      lng,
      address_line: null,
  ...
  // Copy coordinates to clipboard capability inside context menu
  ...
  // Dismiss context menu interaction
  useEffect(() => {
  ...
  // Retrieves both active reports and checkpoints in single or parallel calls
  ...
  const handleAdd = async (cpData) => {
    try {
      ...
      // Refresh map data to show new pin
      fetchMapData()
    } catch (err) {
  ...
  const handleUpdate = async (cpData) => {
    try {
      ...
      // Refresh map data to update status/info
      fetchMapData()
    } catch (err) {

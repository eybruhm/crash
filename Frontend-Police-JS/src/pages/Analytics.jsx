 ...
 * ANALYTICS PAGE
 * ============================================================================
 * Purpose: Visualizes crime statistics and trends for command decision making.
 * Features:
 * - Hotspot analysis by location (City/Barangay)
 * - Crime category distribution
 * - Overview metrics for resolution time
 * - PDF Export capability
 * - Date range and scope filtering (local vs all offices)
 * ============================================================================
 */
...
const Analytics = () => {
  ...
  // Loads all analytics subsections in parallel
  useEffect(() => {
    if (!isAuthenticated) {
    ...
    const fetchAnalytics = async () => {
      try {
        ...
        // ensures the dropdown contains all valid options
  ...
  // Resets barangay filter when city changes
  useEffect(() => {
    if (cityFilter === 'all') {
    ...
    // Fallback only: derive from displayed top locations if backend list is missing
  ...
  // Leverages the backend PDF generation endpoint
  const handleExport = () => {
    const filters = buildFilters()
  ...
  // Cache Rebuild Logic
  // Prevents abuse by enforcing a cooldown period
  ...
  const handleRebuildCache = async () => {
    ...
    try {
      ...
      // Refresh analytics data after rebuild to show new numbers immediately
      const filters = buildFilters()
      const [hotspotsData, categoriesData, overviewData] = await Promise.all([
        getLocationHotspots(filters),
        getCategoryStats(filters),
      ...
      // ... (re-process logic same as main effect)
      // Note: Duplicated processing logic for brevity here, ideally extracted to helper
      const topLocations = (hotspotsData.results || []).map(loc => ({
        city: loc.location_city || 'Unknown',
        barangay: loc.location_barangay || 'Unknown',
        count: loc.report_count || 0,

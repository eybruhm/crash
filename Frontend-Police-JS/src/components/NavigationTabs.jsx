 ...
 * ============================================================================
 * Purpose: Provides main navigation links for the application.
 * Features:
 * - Highlights active tab based on current route or prop
 * - Uses React Router for transitions
 * Input:
 * - activeTab: Optional string to force active state (overrides URL)
 * ============================================================================
 */
...
const NavigationTabs = ({ activeTab }) => {
  ...
  // Determine active tab: prefer prop, fallback to URL path
  const current = activeTab || location.pathname.replace('/', '') || 'dashboard'

  // Helper to generate CSS classes for tabs
  // Applies 'tab-active' style if the tab ID matches current route
  const tabClass = (tab) =>

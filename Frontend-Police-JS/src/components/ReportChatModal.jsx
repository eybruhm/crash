 ...
 * REPORT CHAT MODAL
 * ============================================================================
 * Purpose: Real-time messaging interface between police and reporter.
 * Features:
 * - Polling-based message synchronization (every 5s)
 * - Optimistic UI updates for immediate feedback
 * - Automatic scrolling to latest message
 * - Error handling for failed sends
 * Input:
 * - report: The active report context (ID, reporter details)
...
const ReportChatModal = ({ report, onClose }) => {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      ...
      // Only show error on initial load to avoid noise during polling
      if (loading) {
  ...
  // Setup polling cycle
  // Fetches fresh messages at defined interval
  useEffect(() => {
    fetchMessages()
  ...
  // Timestly formatter
  // Outputs: "MM/DD - HH:mm" in Philippine time
  ...
  // Uses optimistic updates to update UI before server confirmation
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      setMessageText(text) // Restore input
    } finally {
      setSending(false)
    }
  }

 ...
 * ============================================================================
 * Purpose: Display navigation information for a police officer to reaching an incident.
 * Features:
 * - Shows Google map with route from station to incident
 * - Fetches route data from backend (includes QR code, and directions link)
 * - Calculates distance and ETA (via Backend or Google Maps SDK or Haversine fallback)
 * - Provides 'Open in Google Maps' deep link
 * ============================================================================
 */
 ...
 * Used as fallback when Google Directions API fails or backend data is missing
 */
...
const DirectionsModal = ({ report, onClose, onDistanceEtaUpdate }) => {
  ...
  // Defaults to generic Manila coordinates if missing
  ...
  // This endpoint provides the official route/QR tracking logic
  useEffect(() => {
    const fetchRouteFromBackend = async () => {
      ...
      try {
        ...
        // Continue with frontend fallback (haversine/google sdk)
      } finally {
        setLoading(false)
      }
    }
  ...
  // Runs if backend data or google SDK data is unavailable
  useEffect(() => {
  ...
  // Uses the client-side Google Maps Directions Service
  useEffect(() => {

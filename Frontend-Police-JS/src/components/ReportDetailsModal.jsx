 ...
 * ============================================================================
 * Purpose: Detailed view for an active report, allowing status management.
 * Features:
 * - Displays report metadata (reporter, location, category)
 * - Shows media attachments (images/videos)
 * - Allows updating report status (Pending -> Resolved)
 * - Calculates distance/ETA if backend data is missing
 * Input:
 * - report: Active report object
 * - onStatusChange: Callback for status updates
 * - onOpenChat: Callback to launch chat modal
 * ============================================================================
 */
...
// Configuration for possible report statuses
...
const ReportDetailsModal = ({ report, onClose, onStatusChange, onOpenChat, onDistanceEtaUpdate }) => {
  // Local state for editing status and remarks
  ...
  // Sync local state when report props change
  useEffect(() => {
    setSelectedStatus(report.status || 'Pending')
    setRemarks(report.remarks || '')
  }, [report.status, report.remarks])

  // Fallback distance/ETA calculation
  // Runs if the report object doesn't already have pre-calculated values
  // Uses simplified Haversine formula + assumed city speed (35km/h)
  useEffect(() => {
    ...
    // Estimate mins: (distance / speed) + overhead
  ...
  // Fetch complete media list from server
  // This ensures we have the latest photos/videos uploaded by the reporter
  useEffect(() => {
  ...
  // Triggers the parent callback to persist changes to backend
  const handleStatusChange = () => {
    if (selectedStatus !== report.status && onStatusChange) {
      ...
      // Allow saving if only remarks changed, even if status is same
      onStatusChange(report.id, selectedStatus, remarks)
    }
    onClose()
  }
  ...
  // Visual helper for category icons

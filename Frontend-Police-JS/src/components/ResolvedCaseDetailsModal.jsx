 ...
 * ============================================================================
 * Purpose: Read-only view for historical/resolved cases.
 * Features:
 * - Shows full resolution details (time taken, notes)
 * - Displays original report context
 * - Fetches and displays preserved media attachments
 * Input:
 * - case_: Case object with resolution metadata
...
const ResolvedCaseDetailsModal = ({ case_, onClose }) => {
  ...
  // Fetch comprehensive details on mount
  // Retrieves both the report data and associated media
  useEffect(() => {
    const load = async () => {
      if (!case_?.id) return
      try {
        ...
        // Parallel fetch for details and media could be optimized here,
        // but sequential ensures logical error handling order
  ...
  // Handles differences between summary list data and full detail API response
  ...
  // Sort media by type for display
